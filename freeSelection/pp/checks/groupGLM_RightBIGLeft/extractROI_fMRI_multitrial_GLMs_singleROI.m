...
origData_root = '/imaging/henson/users/ek03/projects/HAROLD/freeSelection/pp/data';

extractingDir = fullfile(origData_root ,'aa_norm_write_dartel_masked');
AnySPMDirWithOnsets = fullfile(origData_root ,'firstLevelModel');
...
parfor s = 1:length(CCIDList)
  ...
  VY = spm_vol(cellstr(spm_select('ExtFPList',fullfile(extractingDir,CCID),'^mswar.*\.nii$')));
  VY = [VY{:}];
  ...
  S.XC = SPMmat.SPM.xX.X(:,5:12); %confounds
  %S.coi = []; not needed
