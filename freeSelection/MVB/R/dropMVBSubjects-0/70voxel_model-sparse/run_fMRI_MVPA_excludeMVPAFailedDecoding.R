...
#CANT TEST AGIANST CHANCE IF DROPPING PPL FOR BEING BELOW!
# #------------------------ DECODING ACCURACY VS CHANCE ------------------------#
# #-- LH vs chance --#
# ggdensity(df_subset$MVPA_L_4way, xlab = "Decoding Accuracy (%)")
# ggqqplot(df_subset$MVPA_L_4way)
# t.test(df_subset$MVPA_L_4way, alternative = "greater", mu = 0.25)
# #non-parametic
# # shapiro.test(df_subset$MVPA_L_4way)
# # wilcox.test(df_subset$MVPA_L_4way, alternative = "greater", mu = 0.25)
# 
# #-- RH vs chance --#
# ggdensity(df_subset$MVPA_R_4way, xlab = "Decoding Accuracy (%)")
# ggqqplot(df_subset$MVPA_R_4way)
# t.test(df_subset$MVPA_R_4way, alternative = "greater", mu = 0.25)
# #non-parametic
# # shapiro.test(df_subset$MVPA_R_4way)
# # wilcox.test(df_subset$MVPA_R_4way, alternative = "greater", mu = 0.25)
# 
# #-- Bilateral vs chance --#
# ggdensity(df_subset$MVPA_Bi_4way, xlab = "Decoding Accuracy (%)")
# ggqqplot(df_subset$MVPA_Bi_4way)
# t.test(df_subset$MVPA_Bi_4way, alternative = "greater", mu = 0.25)
# #non-parametic
# # shapiro.test(df_subset$MVPA_Bi_4way)
# # wilcox.test(df_subset$MVPA_Bi_4way, alternative = "greater", mu = 0.25)
...
#   decAccMean = c(mean(df_subset$MVPA_L_4way),mean(df_subset$MVPA_R_4way),mean(df_subset$MVPA_R_4way)),
#   decAccSE = c(se(df_subset$MVPA_L_4way),se(df_subset$MVPA_R_4way),se(df_subset$MVPA_R_4way)),
...
#   scale_x_discrete(labels = df_subset2$ROI) +
...
# print(p)
# 
# ggsave(file.path(outImageDir,'decAcc_chance_excludeMVPA.png'), plot = p,
...
# 
# rm(p)
# png(file.path(outImageDir,'decAcc_Age_LH_excludeMVPA.png'), width = 500, height = 500)
# 
# p <- ggplot(df_subset, aes(x = age, y = MVPA_L_4way))
...
# #Plot
# rm(p)
# png(file.path(outImageDir,'decAcc_Age_RH_excludeMVPA.png'), width = 500, height = 500)
# 
# p <- ggplot(df_subset, aes(x = age, y = MVPA_R_4way))
# p <- p + geom_point(shape = 21, size = 3, colour = "indianred2", fill = "lightpink", stroke = 2)
# p <- p + stat_smooth(method = "rlm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
# #p <- p + stat_smooth(method = "rlm", se = TRUE, fill = "grey60", formula = y ~ poly(x,2, raw = TRUE), colour = "springgreen3", size = 3)
# p <- p + geom_hline(yintercept = 25, linetype = "longdash", color = 'black', size = 0.5) #chance
