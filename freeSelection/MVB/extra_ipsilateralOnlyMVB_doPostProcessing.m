...
RDirSubFolder = fullfile('extra_ipsilateralOnlyMVB','70voxel_model-sparse');
...
% RDirSubFolder = fullfile('extra_SMA','70voxel_model-sparse_controlVoxelSize-constrictBilateral');
...
% RDirSubFolder = fullfile('extra_SMA','70voxel_model-sparse_controlVoxelSize-enlargeContralateral');
...
for s = 1:length(CCIDList)
  ...
  fN = sprintf('data/%s/MVB_%s_con1_Action-Baseline_model-%s.mat',CCID,ROINames{2},model);
  load(fN,'MVB');
  F_R(s) = max(MVB.M.F(2:end) - MVB.M.F(1));
  
  fN = sprintf('data/%s/MVB_%s_con1_Action-Baseline_model-%s.mat',CCID,ROINames{3},model);
  load(fN,'MVB');
  F_Bi(s) = max(MVB.M.F(2:end) - MVB.M.F(1));
  
  F_diff(s) = F_Bi(s) - F_R(s);
...
%Use this version if control analysis as we have no univariate for halved bilateral etc.:
% d = table(CCIDList,age,agez,age0z,genderC,genderNum, ...
% inScanner_RTmean,inScanner_RTsd,inScanner_RTidxToExclude, ...
% outScanner_RTmean,outScanner_RTsd,outScanner_RTidxToExclude, ...
% groupFvals,idx_couldNotDecode, ...
