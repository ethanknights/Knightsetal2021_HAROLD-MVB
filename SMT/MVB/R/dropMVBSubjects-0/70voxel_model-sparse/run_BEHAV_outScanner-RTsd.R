...
full <-           lmBF(outScanner_RTsd ~ univariateMean_R_0z * age0z, data = df)
noInteraction <-  lmBF(outScanner_RTsd ~ univariateMean_R_0z + age0z, data = df)
...
p <- p + 
  ...
  theme(panel.border = element_blank(),
        ...
        axis.ticks = element_line(colour = "black",
                                  size = 1.5),
        text = element_text(size=24)) +
  coord_cartesian(xlim = c(-1,2) , ylim = c(0,350), expand = TRUE)
