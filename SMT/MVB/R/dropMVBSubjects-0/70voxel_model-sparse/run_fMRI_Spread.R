#======================= Multivariate Spread ======================#
#-------------- LH --------------#
rlm_model <- rlm(spread_LH ~ age0z2, 
...
p <- ggplot(df, aes(x = age, y = spread_LH)) +
...
p <- p + 
  ...
  theme(panel.border = element_blank(),
        ...
        axis.ticks = element_line(colour = "black",
                                  size = 1.5),
        text = element_text(size=24))
ggsave(file.path(outImageDir,'multivariateSpread_RH.png'),
       width = 25, height = 25, units = 'cm', dpi = 300); p
...
#EXTRA
#======
#Is RH Spread redundant with LH spread?
#i.e. is more info in RH with age, already captured by more info in LH with age?

#-- Correlation of LH & RH Spread --#
library("ggpubr")
p <- ggscatter(df, x = "spread_LH", y = "spread_RH",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Spread LH", ylab = "Spread RH",
          xlim = c(0,0.03),
          ylim = c(0,0.03))
ggqqplot(df$spread_LH, ylab = "spread_LH")
shapiro.test(df$spread_LH) # => p < .0001
ggqqplot(df$spread_RH, ylab = "spread_RH")
shapiro.test(df$spread_RH) # => p < .0001
cor.test(df$spread_LH,df$spread_RH,method="spearman") #Spearmans rank for non-normally distributed
