...
destDir_root = 'data';
kernelSize = 0; %8 | 10 %value refers to the smoothing of the RSFA scale output smoothing operation (i.e. all images are smoothed by 10mm, then this value too after RSFA scaling)

sourceDirs = {'firstLevelModel','firstLevelModel_smooth'}; 
destDirs = {'groupModel',['groupModel_RightMINUSLeft_smooth_',num2str(kernelSize),'mm']};
...
for i = 2%1:2 %i=1 is wds, i=2 is smooth - DONT really need this for wds
  ...
  for s = 1:nSubs
    CCID = CCIDList{s};
    fN{s} = fullfile(destDir_root,sourceDir,CCID,['final_','s',num2str(kernelSize),'mm_','RightMINUSLeft_RSFAScaled.nii,1']);
  end
